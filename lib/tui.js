...
loadSessions();
out(`${ESC}?1049h${ESC}?25l${ESC}?1000h${ESC}?1006h`); // alt screen, hide cursor, mouse on (SGR coords)
process.on('exit', () => out(`${ESC}?1006l${ESC}?1000l${ESC}?1049l${ESC}?25h`));
process.stdin.setRawMode && process.stdin.setRawMode(true);
process.stdin.resume();
// Mouse frames must never leak bytes into handleKey: a legacy X10 click at column 81
// encodes as byte 113 = 'q' and quits the app. Tokenize the stream, buffering partials.
let pending = '';
process.stdin.on('data', buf => {
  let s = pending + buf.toString('utf8');
  pending = '';
  while (s.length) {
    const sgr = s.match(/^\x1b\[<(\d+);(\d+);(\d+)([Mm])/); // SGR: ESC [ < b ; x ; y M|m
    if (sgr) {
      s = s.slice(sgr[0].length);
      const b = Number(sgr[1]);
      if (sgr[4] === 'm' || (b & 3) === 3 || b >= 64) continue; // release / wheel
      clickRow(Number(sgr[3]));
      continue;
    }
    if (s.startsWith('\x1b[<') || s === '\x1b' || s === '\x1b[') {
      pending = s; return;                  // incomplete escape/mouse frame — wait for the rest
    }
    if (s.startsWith('\x1b[M')) {           // legacy X10 frame: ESC [ M b x y
      if (s.length < 6) { pending = s; return; }
      const b = s.charCodeAt(3) - 32;
      if ((b & 3) !== 3 && b < 64) clickRow(s.charCodeAt(5) - 32);
      s = s.slice(6);
      continue;
    }
    if (s[0] === '\x1b') { handleKey(s); return; } // other escape sequence, keep whole
    handleKey(s[0]);
    s = s.slice(1);
  }
});
